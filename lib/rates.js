...
// Текст ошибки для /api/rates. Node прячет сетевую причину за «fetch failed»
// (e.cause: ENOTFOUND, ECONNRESET, ETIMEDOUT, ошибка TLS…) — без неё по
// ответу с сервера не понять, заблокирован источник или просто недоступен.
const errText = e => {
...
function parseValutaMembers(html) {
  ...
  // Страница без таблицы — это не «ноль участников», а другая страница
  // (защита от ботов, заглушка, редирект). Молчаливый [] выглядел на сайте
  // как «нет данных» без причины; в ошибку кладём заголовок страницы.
  if (listStart < 0) {
    const title = (html.match(/<title[^>]*>([\s\S]*?)<\/title>/i) || [])[1];
    // Короткий ответ — это и есть текст блокировки, показываем его целиком
    const hint = title ? `«${stripTags(title).slice(0, 80)}»` : html.length <= 200 ? `${html.length} байт: «${stripTags(html).slice(0, 120)}»` : `${html.length} байт`;
    throw new Error(`valuta.kg: нет таблицы курсов (${hint})`);
  }
...
async function fetchCrypto() {
  return Promise.all(EXCHANGES.map(async ex => {
    try {
      ...
      return { name: ex.name, url: ex.url, note: ex.note, error: errText(e) };
    }
  }));
}
...
async function fetchUsdRub() {
  return Promise.all(USDRUB_SOURCES.map(async src => {
    try {
      ...
      return { name: src.name, url: src.url, error: errText(e) };
    }
  }));
}
...
const settle = async fn => { try { return await fn(); } catch (e) { return { error: errText(e) }; } };
