// Сбор курсов: valuta.kg (НБКР, средний/лучший курс Бишкека, банки и обменки)
...
// Таблица "покупка / продажа" из шапки главной страницы по заголовку блока
function parseKursTable(html, title, codes) {
  const start = html.indexOf(title);
  if (start < 0) throw new Error(`блок «${title}» не найден`);
  const table = html.slice(start, html.indexOf('</table>', start));
  const rows = [...table.matchAll(/<tr>([\s\S]*?)<\/tr>/g)]
    .map(m => [...m[1].matchAll(/<td>([\s\S]*?)<\/td>/g)].map(td => num(stripTags(td[1]))))
    .filter(cells => cells.length === 2);
  const out = {};
  codes.forEach((code, i) => { if (rows[i]) out[code] = { buy: rows[i][0], sell: rows[i][1] }; });
  return out;
}
...
async function fetchValutaHome() {
  ...
  // Порядок валют в шапке: usd, eur, rub, kzt, cny, gbp
  const headStart = html.indexOf('kurs-bar__item--rate-names');
  const head = html.slice(headStart, html.indexOf('</table>', headStart));
  const headCodes = [...head.matchAll(/class="rate-name[^"]*"[^>]*>([a-z]{3})</g)].map(m => m[1].toUpperCase());

  const average = parseKursTable(html, 'Средний курс в Бишкеке', headCodes);
  const best = parseKursTable(html, 'Лучший курс в Бишкеке', headCodes);
...
async function fetchAllRates() {
  const [home, nbkr, crypto] = await Promise.all([settle(fetchValutaHome), settle(fetchNbkr), fetchCrypto()]);
  return {
    updatedAt: new Date().toISOString(),
    nbkr,
    bishkek: home.error ? { error: home.error } : { average: home.average, best: home.best },
